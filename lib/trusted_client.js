...
function TrustedClient(options) {
	...
	var baseUrl = url.parse(options.baseUrl)
	, that = this
	, state = {}
	...
	this._log('info', 'new TrustedClient instance for baseUrl: '.concat(baseUrl));
}
...
Object.defineProperties(TrustedClient.prototype, {
	...
	_parseLink: {
		value: function(link) {
			...
			, linkPart = decoded.slice(1, 2).split('/').filter(nonEmpty)
