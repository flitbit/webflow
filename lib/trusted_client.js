...
function TrustedClient(options) {
	...
	if (log) this._log('info', 'new TrustedClient instance for baseUrl: '.concat(baseUrl));
}
	...
	_filterHeaders: {
		value: function(head) {
			...
			Object.keys(head).forEach(function (k) {
				if (this.filtered_headers.indexOf(k) < 0) {
					switch (k) {
						case 'content-length':
						...
						break;
						case 'link':
						o.links = this._parseLinks(head[k]);
						break;
						default:
						o[k] = head[k];
						break;
		...
		value: function(method, req, cb, err, res, body) {
			...
			if (err) {
				...
				try {
					...
					else if (contentType.match(/^application\/binary/)) {
						result.body = new Buffer(part.body, 'binary');
					} else if (contentType.match(/^application\/octet-stream/)) {
						result.body = new Buffer(part.body, 'binary');
					} else if (contentType.match(/^multipart\/mixed/)) {
						this._parseMultipartResponse(body, result);
	...
	_request: {
		value: function(req, callback) {
			...
			if (req.server_path) {
				...
				basePath = url.format(this.basePath);
			}
			...
			if (this._state.httpSignature) opt.httpSignature = this._state.httpSignature;
